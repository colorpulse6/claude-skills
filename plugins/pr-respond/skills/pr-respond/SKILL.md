...
Use the exact templates in `references/replies-and-edge-cases.md`: Fix replies
carry the short SHA + a line-anchored link to the new commit (offer
Post/Customize/Skip before each); Push back replies are the user's reasoning
verbatim with no preamble; Defer replies are `⏸️ Deferred: {reason}`.
...
Seven known edge cases (dirty tree, failed push, failed type check, reply
chains, unparseable comments, monorepo tsconfig scope, checkout conflicts) are
handled per `references/replies-and-edge-cases.md` — consult it the moment one
appears.
